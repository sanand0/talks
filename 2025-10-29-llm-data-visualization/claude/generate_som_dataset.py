...
def generate_customer_segment(n_customers, base_params, noise_level=0.15):
    ...
    for _ in range(n_customers):
        customer = {
            'monthly_spending': max(0, np.random.normal(
                base_params['monthly_spending'], 
                base_params['monthly_spending'] * noise_level
            )),
            'visit_frequency': max(0, np.random.normal(
                base_params['visit_frequency'], 
                base_params['visit_frequency'] * noise_level
            )),
            'avg_purchase_value': max(0, np.random.normal(
                base_params['avg_purchase_value'], 
                base_params['avg_purchase_value'] * noise_level
            )),
            'online_preference': np.clip(np.random.normal(
                base_params['online_preference'], 
                noise_level
            ), 0, 1),
            'discount_sensitivity': np.clip(np.random.normal(
                base_params['discount_sensitivity'], 
                noise_level
            ), 0, 1)
        }
...
for segment_name, segment_info in segments.items():
    customers = generate_customer_segment(
        segment_info['count'],
        segment_info['params']
    )
...
df = df[['customer_id', 'monthly_spending', 'visit_frequency', 
         'avg_purchase_value', 'online_preference', 'discount_sensitivity', 
         'customer_segment']]
