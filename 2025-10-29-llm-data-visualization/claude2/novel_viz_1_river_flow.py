...
for idx, row in df_norm.iterrows():
    ...
    lc = LineCollection(segments, colors=color, linewidths=base_width, alpha=0.4, capstyle="round")
...
for segment, offset in segment_offsets.items():
    ...
    ax_main.text(
        ...
        bbox=dict(
            ...
            facecolor="white",
            edgecolor=segment_colors[segment],
            linewidth=2,
        ),
    )
...
ax_main.annotate(
    ...
    xy=(2, 20),
    xytext=(1, 5),
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#fff3cd", edgecolor="#ff6b6b", linewidth=2),
...
ax_main.annotate(
    ...
    xy=(7, 45),
    xytext=(8.5, 55),
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#d3f9d8", edgecolor="#2ecc71", linewidth=2),
...
ax_main.annotate(
    ...
    xy=(4, 75),
    xytext=(2.5, 85),
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#e7f5ff", edgecolor="#3498db", linewidth=2),
...
ax_main.set_xlabel(
    "Customer Journey Through Feature Space", fontsize=16, fontweight="bold", color="#2c3e50"
)
ax_main.set_ylabel(
    "Customer Flow (grouped by segment)", fontsize=16, fontweight="bold", color="#2c3e50"
)
ax_main.set_title(
    "FEATURE RIVER FLOW VISUALIZATION\n"
...
for segment, color in segment_colors.items():
    legend_elements.append(
        Rectangle(
            (0, 0), 1, 1, fc=color, alpha=0.6, edgecolor="black", linewidth=1.5, label=segment
        )
    )
...
plt.tight_layout()
plt.savefig(
    "/mnt/user-data/outputs/novel_viz_1_river_flow.png",
    dpi=300,
    bbox_inches="tight",
    facecolor="white",
)
