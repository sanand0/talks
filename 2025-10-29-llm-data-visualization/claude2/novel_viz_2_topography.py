...
# Normalize terrain
terrain_smooth = (terrain_smooth - terrain_smooth.min()) / (
    terrain_smooth.max() - terrain_smooth.min()
)

# Plot terrain as contour map
contours = ax_main.contourf(X_grid, Y_grid, terrain_smooth, levels=20, cmap="terrain", alpha=0.7)
contour_lines = ax_main.contour(
    X_grid, Y_grid, terrain_smooth, levels=10, colors="black", alpha=0.2, linewidths=0.5
)

# Add colorbar for elevation
cbar = plt.colorbar(contours, ax=ax_main, pad=0.02)
cbar.set_label(
    "Strategic Value Density\n(Customer Spending × Density)",
    fontsize=12,
    fontweight="bold",
    rotation=270,
    labelpad=25,
...
ax_main.annotate(
    ...
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#fff3cd", edgecolor="#f39c12", linewidth=2.5),
...
ax_main.annotate(
    ...
    xytext=(valley_x - 3, valley_y - 2),
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#ffe0e0", edgecolor="#e74c3c", linewidth=2.5),
...
ax_main.annotate(
    ...
    xytext=(cluster_x + 2.5, cluster_y + 2.5),
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#ffebee", edgecolor="#e74c3c", linewidth=2.5),
    arrowprops=dict(arrowstyle="->", color="#e74c3c", lw=2.5),
    fontsize=11,
    fontweight="bold",
    color="#c0392b",
...
ax_main.annotate(
    "N", xy=(compass_x, compass_y), fontsize=14, fontweight="bold", ha="center", va="bottom"
)
ax_main.plot([compass_x, compass_x], [compass_y - 0.8, compass_y - 0.3], "k-", linewidth=2)
...
ax_profile.axhline(
    y=elevation_slice.mean(), color="red", linestyle="--", linewidth=2, label="Average"
)
ax_profile.legend()
...
bars1 = ax_quadrant.bar(
    x_pos - width / 2, quad_counts, width, label="Customer Count", color="#3498db", alpha=0.8
)
ax_quadrant2 = ax_quadrant.twinx()
bars2 = ax_quadrant2.bar(
    x_pos + width / 2,
    np.array(quad_values) / 1000,
    width,
    label="Total Value ($k)",
    color="#e74c3c",
    alpha=0.8,
...
plt.tight_layout()
plt.savefig(
    "/mnt/user-data/outputs/novel_viz_2_topography.png",
    dpi=300,
    bbox_inches="tight",
    facecolor="white",
)
